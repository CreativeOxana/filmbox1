...
let film;
filmy.forEach((comparedFilm) => {
  if (comparedFilm.id === filmId) {
    film = comparedFilm;
  }
});
detailElm.querySelector('.card-title').textContent = film.nazev;
detailElm.querySelector('.card-text').textContent = film.popis;
const plakat = detailElm.querySelector('.img-fluid');
...
const premiere = dayjs(film.premiera);
const today = dayjs();
const dayDiff = premiere.diff(today, 'days');
let dayString;

if (dayDiff === 0) {
  premiereElm.innerHTML = `Premiéra <strong>${premiere.format(
    'D. M. YYYY',
  )}</strong>, což je dnes.`;
} else if (today.isAfter(premiere)) {
  if (dayDiff === -1) {
    dayString = 'dnem';
  } else {
    dayString = 'dny';
  ...
  premiereElm.innerHTML = `Premiéra <strong>${premiere.format(
    'D. M. YYYY',
  )}</strong>, což bylo před ${-premiere.diff(today, 'days')} ${dayString}.`;
} else {
  if (dayDiff === 1) {
    dayString = 'dnem';
  ...
  premiereElm.innerHTML = `Premiéra <strong>${premiere.format(
    'D. M. YYYY',
  )}</strong>, což bude za ${premiere.diff(today, 'days')} ${dayString}.`;
}

const playElm = document.querySelector('#prehravac');
const controlsElm = playElm.querySelector('.player-controls');
const videoElm = playElm.querySelector('video');
const currentTimeElm = playElm.querySelector('.current-time');
playElm.querySelector('.play').addEventListener('click', () => {
  videoElm.play();
});
playElm.querySelector('.pause').addEventListener('click', () => {
  videoElm.pause();
});
...
videoElm.addEventListener('timeupdate', () => {
  const totalSeconds = Math.round(videoElm.currentTime);
  const seconds = (totalSeconds % 60).toString().padStart(2, '0');
  const minutes = Math.floor(totalSeconds / 60)
    .toString()
    .padStart(2, '0');
  currentTimeElm.textContent = `${minutes}:${seconds}`;
});
...
const starMouseEnter = (event) => {
  setStars(Number(event.target.textContent));
};
...
const starMouseClick = (event) => {
  star = Number(event.target.textContent);
  setStars(star);
};
...
stars.forEach((star) => {
  ...
  star.addEventListener('click', starMouseClick);
});

const formForNoteElm = document.querySelector('#note-form');
formForNoteElm.addEventListener('submit', (event) => {
  event.preventDefault();
  const textFieldElm = formForNoteElm.querySelector('#message-input');
  if (textFieldElm.value.length === 0) {
    textFieldElm.classList.add('is-invalid');
    textFieldElm.focus();
    return;
  }
  const termsElm = formForNoteElm.querySelector('#terms-checkbox');
  if (termsElm.checked === false) {
    termsElm.classList.add('is-invalid');
    termsElm.focus();
    return (termsElm.textContent = 'Děkujeme');
  }

  const textContent =
    'Děkujeme za Vaše hodnocení. Vaše hodnocení bylo odesláno. Přejeme hezký den. ';
  formForNoteElm.innerHTML = `<p class="card-text">${textContent}</p>`;
});
